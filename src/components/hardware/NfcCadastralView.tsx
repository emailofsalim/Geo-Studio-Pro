      ...
      <div className="p-5 bg-[#111111] rounded-2xl border border-white/[0.08] flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-white">NFC Cadastral Survey Monument Interface</h3>
            <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-[#c9a063]/10 text-[#c9a063] border border-[#c9a063]/25">
              NDEF RFID / NFC
            </span>
          </div>
          <p className="text-xs text-white/50">{nfcStatus}</p>
        </div>

        <div className="flex items-center flex-wrap gap-2">
          <button
            onClick={handleStartNfcScan}
            className="px-4 py-2 rounded-lg bg-[#c9a063] hover:bg-[#d6b074] text-black text-xs font-semibold transition-colors flex items-center gap-1.5"
          >
            <Smartphone className="w-3.5 h-3.5" /> Tap / Scan NFC Monument
          </button>
          <button
            onClick={handleSimulateNfcRead}
            className="px-3 py-2 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] text-white/80 hover:text-white text-xs font-medium transition-colors border border-white/[0.08] flex items-center gap-1.5"
          >
            <Radio className="w-3.5 h-3.5 text-blue-400" /> Simulate Tag Tap
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
        ...
        <div className="md:col-span-6 bg-[#111111] p-5 rounded-2xl border border-white/[0.08] space-y-4 font-mono">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-white font-sans">Decoded Tag Information</span>
            {nfcLastRead && (
              <button
                onClick={() => {
          ...
          {nfcLastRead ? (
            <div className="space-y-3">
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center justify-between">
                <div>
                  <div className="text-sm font-bold text-emerald-400">{nfcLastRead.pointId}</div>
                  <div className="text-[10px] text-emerald-200/70">{nfcLastRead.surveyType}</div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="p-2.5 bg-white/[0.02] border border-white/[0.06] rounded-lg">
                  <div className="text-[10px] text-white/40">UTM Easting</div>
                  <div className="text-xs font-bold text-white mt-0.5">{nfcLastRead.utmEasting.toFixed(3)} mE</div>
                </div>
                <div className="p-2.5 bg-white/[0.02] border border-white/[0.06] rounded-lg">
                  <div className="text-[10px] text-white/40">UTM Northing</div>
                  <div className="text-xs font-bold text-white mt-0.5">{nfcLastRead.utmNorthing.toFixed(3)} mN</div>
                </div>
                <div className="p-2.5 bg-white/[0.02] border border-white/[0.06] rounded-lg">
                  <div className="text-[10px] text-white/40">Elevation (RL)</div>
                  <div className="text-xs font-bold text-[#c9a063] mt-0.5">{nfcLastRead.altitudeM.toFixed(3)} m</div>
                </div>
                <div className="p-2.5 bg-white/[0.02] border border-white/[0.06] rounded-lg">
                  <div className="text-[10px] text-white/40">Surveyor</div>
                  <div className="text-xs font-bold text-white/90 mt-0.5 truncate">{nfcLastRead.surveyor}</div>
                </div>
              </div>

              <div className="p-2.5 bg-white/[0.02] border border-white/[0.06] rounded-lg text-xs">
                <div className="text-[10px] text-white/40">Field Notes / Monography</div>
                <div className="text-xs text-white/80 mt-1">{nfcLastRead.notes || 'No notes attached.'}</div>
              </div>

              {onSendToGis && (
                <button
                  onClick={() => onSendToGis(nfcLastRead)}
                  className="w-full py-2 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] text-white text-xs font-medium transition-colors flex items-center justify-center gap-1.5 border border-white/[0.08]"
                >
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Send to GIS Map Layer
                </button>
              )}
            </div>
          ) : (
            <div className="p-8 text-center border border-dashed border-white/10 rounded-xl space-y-2">
              <Cpu className="w-8 h-8 text-white/20 mx-auto" />
              <div className="text-xs text-white/40">No NFC Survey Marker scanned yet.</div>
            </div>
          )}
        </div>

        {/* Program / Write Tag Form */}
        <div className="md:col-span-6 bg-[#111111] p-5 rounded-2xl border border-white/[0.08] space-y-3 font-mono">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-white font-sans">Program On-Site NFC Monument</span>
            <Upload className="w-4 h-4 text-[#c9a063]" />
          </div>

          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-white/40 block mb-1">Monument ID</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-[10px] text-white/40 block mb-1">Survey Type</label>
              <select
                value={nfcWriteForm.surveyType}
                onChange={e => setNfcWriteForm({ ...nfcWriteForm, surveyType: e.target.value as any })}
                className="w-full px-2.5 py-1.5 rounded-lg bg-[#161616] border border-white/[0.08] text-xs text-white"
              >
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-white/40 block mb-1">Elevation / RL (m)</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-[10px] text-white/40 block mb-1">Surveyor Name / ID</label>
              <input
                type="text"
          ...
          <div>
            <label className="text-[10px] text-white/40 block mb-1">Field Description / Monument Reference</label>
            <input
              type="text"
          ...
          <button
            ...
            className="w-full py-2.5 rounded-lg bg-[#c9a063] hover:bg-[#d6b074] text-black text-xs font-semibold transition-colors flex items-center justify-center gap-1.5 pt-2"
          >
